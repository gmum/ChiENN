...
from graphgps.dataset.utils import get_ranking_accuracies
...
class CustomLogger(Logger):
    def __init__(self, name='train', task_type=None, df=None):
    ...
    def regression_rank(self):
        ...
        self.df["outputs"] = pred
        self.df["targets"] = true
        margins, ranking_accuracy, _, _ = \
            get_ranking_accuracies(self.df, mode='>=')  # >= is the mode reported in ChIRo paper
        regression.update({f'ranking_accuracy_{m:.1f}': reformat(a) for m, a in zip(margins, ranking_accuracy)})
        return regression
    ...
    def write_epoch(self, cur_epoch):
        ...
        elif self.task_type == 'regression_rank':
            task_stats = self.regression_rank()
        else:
...
def create_logger(dataframes):
    ...
    for i, dataset in enumerate(range(cfg.share.num_splits)):
        loggers.append(CustomLogger(name=names[i], task_type=infer_task(), df=dataframes[i]))
    return loggers

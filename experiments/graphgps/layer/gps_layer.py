...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
                 ...
                 bigbird_cfg=None, add_chienn_layer=False):
        super().__init__()
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            ...
            if self.local_gnn_type == 'CustomGatedGCN':
                ...
                elif self.local_gnn_type == 'ChiENN':
                    h_local = self.local_model(batch)
                else:
                    h_local = self.local_model(h, batch.edge_index, batch.edge_attr)
                if self.chienn_layer is not None:
                    batch.x = h_local
                    h_local = self.chienn_layer(batch)
        ...
        if self.self_attn is not None:
            ...
            elif self.global_model_type == "GeometricTransformer":
                h_attn = self.self_attn(h_dense, h_dense, h_dense,
                                        mask=mask,
                                        src_distances=batch.distances,
                                        zero_distances_mask=batch.zero_distances_mask)[mask]
            else:
